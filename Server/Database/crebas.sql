...
BEGIN TRANSACTION
	BEGIN TRY
		...
		DECLARE @LOAITK NVARCHAR(100), @MA INT
		SELECT @LOAITK = t.LOAITK, @MA = t.MA FROM TAIKHOAN t WHERE t.UNAME = @UN AND t.PWD = @PW
		PRINT @LOAITK
		IF (@LOAITK = 'KHACHHANG')
		BEGIN
        	SELECT @LOAITK AS 'LOAITK', @MA AS 'MA', KH_MSSV AS 'MSSV' FROM KHACHHANG WHERE KH_ID = @MA
        END
		ELSE IF @LOAITK = 'NHANVIEN'
		BEGIN 
			SELECT @LOAITK AS 'LOAITK', @MA AS 'MA', n.NV_TEN AS 'TEN' FROM NHANVIEN n WHERE n.NV_MA = @MA
		END
		ELSE
			SELECT @LOAITK AS 'LOAITK', @MA AS 'MA'
	END TRY
	BEGIN CATCH
		RAISERROR(977,-1,-1, 'sp_XulyDangNhap');
		ROLLBACK
		RETURN
	END CATCH
COMMIT TRANSACTION
